...
// === CATEGORIAS PADRÃO → Definidas em categories.js ===
...
// === AUTH MODULE (YouTube Membership - Web Only) ===
const SUPABASE_URL = 'https://drbfujpzbptshwuqtkqf.supabase.co';
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRyYmZ1anB6YnB0c2h3dXF0a3FmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQwMTc4NzksImV4cCI6MjA4OTU5Mzg3OX0.8M0oe8ff3wfzDAjAe9ArgbUK47X-M5Wb0uaQiGX-NHk';
...
const auth = {
    ...
    init: async () => {
        ...
        auth.supabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
    ...
    login: async () => {
        ...
        if (statusEl) statusEl.innerText = 'Redirecionando...';
        ...
        if (error) {
            if (statusEl) statusEl.innerText = 'Erro ao conectar: ' + error.message;
        }
    },

    _verifyMembership: async (session) => {
        const statusEl = document.getElementById('login-status');
        if (statusEl) statusEl.innerText = 'Verificando membresia...';

        try {
            const res = await fetch(SUPABASE_URL + '/functions/v1/verify-membership', {
                method: 'POST',
                headers: {
...
window.onload = () => {
    ...
    // INIT AUTH (web = login, APK = direto)
    auth.init();
